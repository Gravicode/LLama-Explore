...
namespace LLama.KernelMemory
{
    internal class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            await RunExampleAsync(MemoryCollectionName, memory);
        }
        private static async Task RunExampleAsync(string MemoryCollectionName, ISemanticTextMemory memory)
        {
            await StoreMemoryAsync(MemoryCollectionName, memory);

            await SearchMemoryAsync(MemoryCollectionName, memory, "How do I get started?");
            ...
            */
            ...
            await SearchMemoryAsync(MemoryCollectionName, memory, "Can I build a chat with SK?");
            ...
            */
            ...
            await SearchMemoryAsync(MemoryCollectionName,memory, "Jupyter notebook");
                                    
            await SearchMemoryAsync(MemoryCollectionName,memory, "README: README associated with a sample chat summary react-based webapp");
                                    
            await SearchMemoryAsync(MemoryCollectionName,memory, "Jupyter notebook describing how to pass prompts from a file to a semantic skill or function");
        }

        private static async Task SearchMemoryAsync(string MemoryCollectionName, ISemanticTextMemory memory, string query)
        {
            Console.WriteLine("\nQuery: " + query + "\n");
        ...
        private static async Task StoreMemoryAsync(string MemoryCollectionName, ISemanticTextMemory memory)
        {
